                ...
                return (
                  <div key={item.id_product || item.id} className="flex justify-between py-3 text-sm">
                    <div>
                      ...
                      <p className="font-semibold text-white line-clamp-1">{item.product_name}</p>
                      {/* 5. DIUBAH: Menampilkan itemQty dan itemPrice */}
                      <p className="text-xs text-white/40">{itemQty}x @ Rp {itemPrice.toLocaleString("id-ID")}</p>
                    </div>
                    <p className="font-medium text-[#c9a84c]">Rp {(itemPrice * itemQty).toLocaleString("id-ID")}</p>
                  </div>
                );
              })
            )}
