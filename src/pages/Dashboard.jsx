...
const statCards = [
  { icon: <FaShoppingCart />, value: "75", label: "Total Orders", change: "+12%", iconBg: "#10b981" },
  { icon: <FaTruck />, value: "124", label: "Total Delivered", change: "+8%", iconBg: "#3b82f6" },
  { icon: <FaBan />, value: "40", label: "Total Canceled", change: "+23%", iconBg: "#ef4444" },
  { icon: <FaDollarSign />, value: "Rp.128", label: "Total Revenue", change: "+18%", iconBg: "#f59e0b" },
];
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 170 }}>
      ...
      {yTicks.map((v) => (
        <g key={v}>
          ...
          <text x={padL - 6} y={toY(v) + 4} textAnchor="end" fontSize="9" fill="#4b5563">{v}M</text>
        </g>
      ))}
      {revenueData.map((d, i) => (
      ...
      <polyline fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="1.5" strokeDasharray="5 4" points={tgtPoints} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-2xl p-5" style={{ background: "#1a1d27", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div className="flex items-center gap-4 mb-3">
              <p className="text-xs" style={{ color: "#6b7280" }}>6 bulan terakhir (juta Rp)</p>
              <div className="flex items-center gap-3 ml-auto">
                <span className="flex items-center gap-1.5 text-xs" style={{ color: "#6b7280" }}>
                  <span className="w-3 h-0.5 inline-block rounded" style={{ background: "#f59e0b" }} />Aktual
                </span>
                <span className="flex items-center gap-1.5 text-xs" style={{ color: "#4b5563" }}>
                  <span className="w-3 h-0.5 inline-block rounded" style={{ background: "#4b5563" }} />Target
                </span>
              </div>
            </div>
            <RevenueChart />
          ...
          <div className="rounded-2xl p-5" style={{ background: "#1a1d27", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <p className="text-xs mb-4" style={{ color: "#6b7280" }}>Unit terjual bulan ini</p>
              ...
              <thead>
                <tr>
                  {["ID Pesanan", "Pelanggan", "Produk", "Total", "Status"].map((h) => (
                    <th key={h} className="text-left font-bold uppercase tracking-wide pb-3"
              ...
              <tbody>
                {recentOrders.map((order) => (
                  <tr key={order.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                    ...
                    <td className="py-2.5 font-bold" style={{ color: "#f1f5f9" }}>{order.total}</td>
                    <td className="py-2.5">
                      <span
                        className="inline-flex items-center gap-1.5 font-bold px-2 py-1 rounded-full"
                        style={{ fontSize: "10px", ...statusStyle[order.status] }}
                      >
                  ...
                  <div>
                    <p className="text-xs leading-relaxed" style={{ color: "#6b7280" }}>
                      {a.text.split(a.bold).map((part, j, arr) =>
                        j < arr.length - 1
                          ? <span key={j}>{part}<span style={{ color: "#d1d5db", fontWeight: 600 }}>{a.bold}</span></span>
                          : part
                      )}
                    </p>
