  ...
  // URL Storage Gambar Backend Laravel Anda
  const IMAGE_BASE_URL = "http://localhost:8000/storage/products/";
  ...
  /* SVG ikon fallback jika gambar default.png atau kosong */
  ...
  export default function KatalogProduk() {
    ...
    // Handler Tambah dengan Form Data Multipart
    const handleAddProduct = async (formDataObj, imageFile) => {
      try {
        const data = new FormData();
        data.append("product_name", formDataObj.product_name);
        data.append("category_id", formDataObj.category_id);
        data.append("price", formDataObj.price);
        data.append("stock", formDataObj.stock);
        ...
        alert("Gagal menambahkan produk, silakan periksa inputan Anda!");
      }
    };

    // Handler Edit dengan Form Data Multipart + Spoofing Method PUT
    const handleEditProduct = async (id_product, formDataObj, imageFile) => {
      try {
        const data = new FormData();
        data.append("product_name", formDataObj.product_name);
        data.append("category_id", formDataObj.category_id);
        ...
        // Menggunakan spoofing _method agar Laravel sanggup membaca kiriman multipart file
        data.append("_method", "PUT");
        if (imageFile) data.append("image", imageFile);

        const response = await api.post(`/products/${id_product}`, data, {
          headers: { "Content-Type": "multipart/form-data" }
        });
  ...
  function GridCard({ p, onDetail }) {
    ...
    return (
      <div onClick={onDetail} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{ background: hov ? C.cardHover : C.card, border:`1px solid ${hov ? C.borderHover : C.border}`, borderRadius:12, padding:"14px 14px 12px", cursor:"pointer", transition:"all .2s", transform: hov ? "translateY(-2px)" : "none", boxShadow: hov ? "0 6px 24px rgba(0,0,0,0.35)" : "none" }} >
        <div style={{ width:"100%", height:120, background:"rgba(255,255,255,0.03)", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:11, overflow:"hidden" }}>
          {p.image && p.image !== 'default.png' ? (
            <img src={`${IMAGE_BASE_URL}${p.image}`} alt={p.product_name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          ) : (
    ...
    return (
      <div onClick={onDetail} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{ display:"grid", gridTemplateColumns:"2fr 1fr 80px 80px 80px 110px 80px", gap:10, padding:"12px 16px", borderRadius:10, background: hov ? C.cardHover : C.card, border:`1px solid ${hov ? C.borderHover : C.border}`, alignItems:"center", cursor:"pointer", transition:"all .18s" }} >
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:34, height:34, borderRadius:8, flexShrink:0, background:"rgba(255,255,255,0.04)", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden" }}>
            {p.image && p.image !== 'default.png' ? (
              <img src={`${IMAGE_BASE_URL}${p.image}`} alt={p.product_name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            ) : (
  ...
  function Modal({ modal, onClose, onAdd, onEdit, onDelete }) {
    ...
    const [imageFile, setImageFile] = useState(null); // State penyimpan file gambar
    ...
    const handleSubmit = (e) => {
      ...
      if (isEditing) {
        onEdit(p.id_product, form, imageFile);
      } else {
    ...
    const renderForm = () => (
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:11, color:C.textSecondary, display:"block", marginBottom:4 }}>Gambar Produk</label>
          <input type="file" accept="image/*" onChange={handleFileChange} style={{ ...inp, padding: "6px 8px" }}/>
        </div>
        ...
        <div style={{ display:"flex", gap:10, marginTop:8 }}>
          ...
          <button type="button" onClick={() => (isEditing ? setIsEditing(false) : onClose())} style={{ flex:1, background:"rgba(255,255,255,0.04)", color:C.textSecondary, border:`1px solid ${C.border}`, borderRadius:9, padding:"10px", fontWeight:600, cursor:"pointer", fontSize:13 }} >
            Batal
          </button>
        </div>
      </form>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"26px 26px 22px", width:420, maxWidth:"92vw", maxHeight:"88vh", overflowY:"auto", position:"relative" }} onClick={e=>e.stopPropagation()}>
          ...
          {isAdd || isEditing ? (
            ...
            <>
              ...
              <div style={{ width:"100%", height:140, background:"rgba(255,255,255,0.03)", borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:14, overflow:"hidden" }}>
                {p.image && p.image !== 'default.png' ? (
                  <img src={`${IMAGE_BASE_URL}${p.image}`} alt={p.product_name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button onClick={() => setIsEditing(true)} style={{ flex:1, background:`linear-gradient(135deg,${C.accent},#d97706)`, color:"#fff", border:"none", borderRadius:9, padding:"9px", fontWeight:600, cursor:"pointer", fontSize:12 }} >
                  ✏️ Edit
                </button>
